...
There are two important data points that we can extract from this data set:
...
```{r map_it, warning=F, message=F, fig.width=10, fig.height=6, quietly=T}
ggplot(states) +
  geom_sf() +
...
So the DMA, or designated market area, doesn't line up with normal state boundaries. 

We want to illustrate not just the locations of these stations with dots but also the saturation of Sinclair stations per DMA. 

So first, we need to count up how many stations there are per DMA, which is simple to do thanks to how the JSON we scraped was structured.
...
```{r dma_map_choropleth, warning=F, message=F, fig.width=10, fig.height=6, quietly=T}
...
We can't label all the dots on the map because there are so many.

So let's create a list of cities that represent a decent distribution.
...
# Cleaning up some of the DMA names
...
```{r sf_map3, warning=F, message=F, fig.width=10, fig.height=8, quietly=T}
